// 将此文件替换为你原来的 planar_move 源文件
#include "my_gazebo_plugins/cmd_vel_z_plugin.hpp" // 若需要，可改为实际头文件名
...
namespace gazebo_plugins
{
    ...
    void GazeboRosPlanarMove::Reset()
    {
        ...
        impl_->target_cmd_vel_.linear.z = 0; // reset z as well
    }
    ...
    void GazeboRosPlanarMovePrivate::OnUpdate(const gazebo::common::UpdateInfo &_info)
    {
        ...
        if (seconds_since_last_update >= update_period_)
        {
            ...
            // -------------------------
            // 关键改动：将 z 分量从 0 改为来自 cmd_vel 的 linear.z
            // 如果你希望 z 速度在 body frame 中，再做变换；通常直接使用 linear.z 即可。
            // -------------------------
            model_->SetLinearVel(
                ignition::math::Vector3d(
                    ...
                    target_cmd_vel_.linear.z)); // <-- 使用 linear.z 控制 z 方向

...
Legged Bringup 是 SCURC 机器人导航仿真系统中的核心启动包，负责集成和协调整个机器人导航栈的启动流程。该包基于 ROS 2 Humble 开发，提供了完整的机器人导航系统配置，包括传感器驱动、定位算法、导航规划、可视化等模块的统一启动和管理。
...
```bash
ros2 launch legged_bringup relocalization.launch.py
...
ros2 launch legged_bringup simulation_bringup.launch.py start_sim:=true mode:=nav
```
...
```bash
ros2 launch legged_bringup octomap_server_intensity.launch.py
```
...
ros2 launch legged_bringup dynamic_waypoint_mission.launch.py start_sim:=false
...
```bash
# 启动FlyStep行为树任务
ros2 launch legged_bringup fly_step_simulation.launch.py
...
ros2 launch legged_bringup simulation_with_yolo_bringup.launch.py start_sim:=true
...
```bash
# 启动KFS智能规划器（单独启动）
ros2 launch legged_bringup kfs_planner.launch.py

# 或使用包装器启动
ros2 launch legged_bringup start_planner.launch.py
```
...
```bash
# 运行KFS决策消息发布器（用于测试）
python3 $(ros2 pkg prefix legged_bringup)/lib/legged_bringup/scripts/publish_kfs.py --timeout 30 --interval 1.0
```
...
# 然后手动启动RViz并加载对应配置文件
rviz2 -d $(ros2 pkg prefix legged_bringup)/share/legged_bringup/rviz/nav2_default_view.rviz
```
...
```bash
# 1. 启动仿真环境和导航
ros2 launch legged_bringup simulation_bringup.launch.py start_sim:=true
...
# 4. 启动包含规划器的完整任务
ros2 launch legged_bringup dynamic_waypoint_mission.launch.py
```
...
# 使用测试地图启动导航
ros2 launch legged_bringup navigation.launch.py \
  map:=$(ros2 pkg prefix legged_bringup)/share/legged_bringup/maps/test_map.yaml
```

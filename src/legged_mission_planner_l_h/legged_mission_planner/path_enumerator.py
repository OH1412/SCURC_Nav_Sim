...
def enumerate_scenarios(
    strategies: Sequence[str] = ('safe_edges', 'middle_paths'),
    switch_modes: Sequence[str] = ('wp4_only', 'wp4_or_wp5'),
...
def export_enumerated(
    ...
    for index, scenario in enumerate(enumerate_scenarios(limit=limit)):
        ...
        name = f"scenario_{index:04d}_{scenario['strategy']}_{scenario['switch_mode']}"
        base = output_dir / name
        exporter.export(sequence, scenario, scenario['strategy'], scenario['switch_mode'], base)
        exported.append(base)
    return exported


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description='Export mission-planner scenario files.')

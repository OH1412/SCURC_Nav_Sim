...
void R2WaypointTask::initialize(
  ...
  const std::string & plugin_name)
{
  ...
  if (!node) {
    RCLCPP_FATAL(rclcpp::get_logger("R2WaypointTask"), "无法锁定父节点，插件初始化失败！");
  ...
  height_sub_ = node->create_subscription<nav_msgs::msg::Odometry>(
    "/state_estimation", 10,
    std::bind(&R2WaypointTask::heightCallback, this, std::placeholders::_1)
  );

  RCLCPP_INFO(logger, "R2航点任务插件初始化完成（名称：%s）", plugin_name.c_str());
}

bool R2WaypointTask::processAtWaypoint(
  const geometry_msgs::msg::PoseStamped & /*curr_pose*/,
  const int & curr_waypoint_index)
{
  // 强制打印确认函数进入（无任何依赖）
  printf("[R2WaypointTask] 【强制打印】进入processAtWaypoint，航点索引：%d\n", curr_waypoint_index);
  fflush(stdout);

  // 探针日志1：使用全局日志器
  RCLCPP_INFO(rclcpp::get_logger("R2WaypointTask"), "【探针1】进入processAtWaypoint，航点索引：%d", curr_waypoint_index);

  // 检查node_有效性（使用安全的bool转换）
  if (!static_cast<bool>(node_)) {
      printf("[R2WaypointTask] 【强制打印】node_ 为空！\n");
      fflush(stdout);
      RCLCPP_ERROR(rclcpp::get_logger("R2WaypointTask"), "【探针2】node_ 为空，跳过航点%d任务", curr_waypoint_index);
      return false;
  }
...
void R2WaypointTask::heightCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  if (!msg) {
    RCLCPP_ERROR(rclcpp::get_logger("R2WaypointTask"), "收到空的里程计消息！");
    return;
  }
  ...
  if (node_) { 
    ...
    RCLCPP_INFO(rclcpp::get_logger("R2WaypointTask"), "收到Z轴高度：%.3fm（node_为空）", current_z_);
  }
}

bool R2WaypointTask::executeAscend200mm()
{
...
bool R2WaypointTask::executeAscend400mm()
{
  if (!node_) return false;
  rclcpp::Logger logger = node_->get_logger();
  if (!vel_pub_ || !clock_) {
...
bool R2WaypointTask::executeDelayedDescend200mm()
{
  if (!node_) return false;
  rclcpp::Logger logger = node_->get_logger();
  if (!vel_pub_ || !clock_) {
...
bool R2WaypointTask::executeDelayedDescend400mm()
{
  if (!node_) return false;
  rclcpp::Logger logger = node_->get_logger();
  if (!vel_pub_ || !clock_) {
...
PLUGINLIB_EXPORT_CLASS(
  r2_waypoint_loader_cpp::R2WaypointTask,

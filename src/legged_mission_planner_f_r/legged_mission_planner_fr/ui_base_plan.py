...
class BaseMissionPlannerUI(PlanningUICommon):
    """Interactive labeling and base mission planning (no quiz flow)."""
    ...
    def _build(self) -> None:
        ...
        ttk.Label(
            header,
            text='标注场上箱子与归位区后点击「规划并导出」。完成后请启动智力题规划 launch。',
            style='Hint.TLabel',
        ...
        ttk.Button(sidebar, text='规划并导出', style='Accent.TButton', command=self._plan_and_export).pack(fill='x', pady=4)
        ...
        # Quiz panel widgets required by PlanningUICommon (hidden in base UI)
        self.quiz_status_label = tk.Label(self.root)
        self.quiz_countdown_label = tk.Label(self.root)
        self.quiz_icon_label = tk.Label(self.root)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description='Front/back base mission planner UI.')
    parser.add_argument('--field-layout', required=True)
    parser.add_argument('--mission-params')
    args = parser.parse_args(argv)
    BaseMissionPlannerUI(args.field_layout, args.mission_params).run()

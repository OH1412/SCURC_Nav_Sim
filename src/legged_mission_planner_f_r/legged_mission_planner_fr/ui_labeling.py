...
from .package_paths import DEFAULT_OUTPUT_DIR, resolve_assets_dir, resolve_config_path
...
class MissionPlannerUI:
    def __init__(self, field_layout: str, mission_params: str | None = None) -> None:
        ...
        self._waypoint_config: WaypointConfig | None = None

        self.root = tk.Tk()
        self.switch_mode = tk.StringVar(self.root, value=self.params.get('default_switch_mode', 'fast_mode'))
        self.zone_order = tk.StringVar(self.root, value='0,1,2,3')
        self.selected_brush = tk.IntVar(self.root, value=0)
        self.root.title('ROBOCON 2026 前后吸取路径规划')
    ...
    def _build(self) -> None:
        ...
        ttk.Label(header, text='ROBOCON 2026 仿生足式机器人 · 前后吸取路径规划', style='Title.TLabel').pack(anchor='w')
        ttk.Label(
            header,
            text='点击左侧物资种类标注场上箱子与归位区；规划含保底路径与主路径 2–5。',
            style='Hint.TLabel',
        ...
        ttk.Combobox(
            sidebar,
            textvariable=self.switch_mode,
            values=['safe_mode', 'fast_mode'],
            state='readonly',
    ...
    def _resolve_waypoint_config(self, switch_mode: str) -> WaypointConfig | None:
        if switch_mode != 'fast_mode':
            return None
        ui_points_cfg = self.params.get('ui_points', {})
        rel_path = ui_points_cfg.get('fast_mode', 'ui_points_fast_mode.yaml')
    ...
    def _build_planner(self, switch_mode: str) -> tuple[MissionPathPlanner, WaypointConfig | None]:
        waypoint_config = self._resolve_waypoint_config(switch_mode)
        if switch_mode == 'fast_mode' and waypoint_config is None:
            raise FileNotFoundError(
                f'fast_mode 需要 ui_points_fast_mode.yaml，查找路径: '
    ...
    def _export_plan(
        ...
        *,
        ...
        if switch_mode == 'fast_mode' and waypoint_config is not None:
            export_quintuple(
                sequence,
                switch_mode,
    ...
    def _plan_and_export(self) -> None:
        try:
            ...
            planner, waypoint_config = self._build_planner(scenario['switch_mode'])
            self._waypoint_config = waypoint_config
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description='Open the front/back mission-planner labeling UI.')
